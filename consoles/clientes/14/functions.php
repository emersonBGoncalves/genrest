<?php function listar_clientes($dados) { require_once('../functions/connection.php');require_once('../functions/functions_basics.php');$conn_cliente = conectaDB('genrest.cwyf2g7tqmmw.sa-east-1.rds.amazonaws.com', 'admin', '123456789', 'genrest_cliente');$clientes_faixa_minimo = isset($dados['clientes_faixa_minimo']) && is_numeric($dados['clientes_faixa_minimo']) ? $dados['clientes_faixa_minimo'] : NULL; $clientes_id = isset($dados['clientes_id']) && is_numeric($dados['clientes_id']) ? $dados['clientes_id'] : NULL; $clientes_nome = !empty($dados['clientes_nome']) ? $dados['clientes_nome'] : NULL; $clientes_ramais = isset($dados['clientes_ramais']) && is_numeric($dados['clientes_ramais']) ? $dados['clientes_ramais'] : NULL; $clientes_sala_conf_minimo = isset($dados['clientes_sala_conf_minimo']) && is_numeric($dados['clientes_sala_conf_minimo']) ? $dados['clientes_sala_conf_minimo'] : NULL; $clientes_sala_conf_num_max = isset($dados['clientes_sala_conf_num_max']) && is_numeric($dados['clientes_sala_conf_num_max']) ? $dados['clientes_sala_conf_num_max'] : NULL; $clientes_faixa_maximo = isset($dados['clientes_faixa_maximo']) && is_numeric($dados['clientes_faixa_maximo']) ? $dados['clientes_faixa_maximo'] : NULL; $query = 'SELECT * FROM clientes WHERE 1 = 1 ';if(!is_null($clientes_faixa_minimo)) { $query .= ' AND clientes_faixa_minimo = "' . $clientes_faixa_minimo . '"';} if(!is_null($clientes_id)) { $query .= ' AND clientes_id = "' . $clientes_id . '"';} if(!is_null($clientes_nome)) { $query .= ' AND clientes_nome = "' . $clientes_nome . '"';} if(!is_null($clientes_ramais)) { $query .= ' AND clientes_ramais = "' . $clientes_ramais . '"';} if(!is_null($clientes_sala_conf_minimo)) { $query .= ' AND clientes_sala_conf_minimo = "' . $clientes_sala_conf_minimo . '"';} if(!is_null($clientes_sala_conf_num_max)) { $query .= ' AND clientes_sala_conf_num_max = "' . $clientes_sala_conf_num_max . '"';} if(!is_null($clientes_faixa_maximo)) { $query .= ' AND clientes_faixa_maximo = "' . $clientes_faixa_maximo . '"';} $result = mysqli_query($conn_cliente, $query);if($result !== FALSE)
						{
